...
// the breaker "opens" for an escalating cooldown — during that window every
// request fast-fails with 503 instead of hammering the upstream. A single
// success resets the failure counter and the backoff. Half-open probing
// happens naturally: once the cooldown expires the breaker closes and the
// next request is a real probe.
//
// The cooldown escalates: 30s → 60s → 120s → 300s, capped at 300s. A fixed
// 30s cooldown produced thousands of failures because DuckDuckGo's VQD token
// acquisition is rate-limited for much longer than 30s — the breaker opened,
// closed after 30s, immediately got 5 more failures, and opened again, in a
// tight loop that never stopped.
export const CB_THRESHOLD = 5;
export const CB_COOLDOWN_MS = 30_000;
export const CB_MAX_COOLDOWN_MS = 300_000;
...
const circuitBreaker: CircuitBreakerState = { failures: 0, openedAt: 0, cooldownMs: CB_COOLDOWN_MS };
...
export function cbRecordFailure(): void {
  ...
  if (circuitBreaker.failures >= CB_THRESHOLD && circuitBreaker.openedAt === 0) {
    ...
    // Escalate: double the cooldown each time the breaker re-opens, capped.
    circuitBreaker.cooldownMs = Math.min(circuitBreaker.cooldownMs * 2, CB_MAX_COOLDOWN_MS);
    console.warn(
      `[DDG-CB] Circuit breaker opened after ${circuitBreaker.failures} consecutive failures — fast-failing for ${circuitBreaker.cooldownMs}ms`
    );
  }
}
...
export function cbRecordSuccess(): void {
  ...
  // Reset cooldown to the base value on success — the upstream is healthy again.
  circuitBreaker.cooldownMs = CB_COOLDOWN_MS;
}
...
export function __setDdgCircuitBreakerStateForTests(failures: number, openedAt: number, cooldownMs?: number): void {
  circuitBreaker.failures = failures;
  circuitBreaker.openedAt = openedAt;
  if (cooldownMs !== undefined) circuitBreaker.cooldownMs = cooldownMs;
}
 ...
 *
 * Thin-gateway note: the session pool and browser-backed chat paths were removed
 * when those services were deleted in the thin-gateway rearchitecture. The core
 * VQD-token + vm-sandbox challenge solver + chat flow is preserved verbatim.
 */
